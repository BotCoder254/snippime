...
const Saved = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      query(
        collection(db, 'savedSnippets'),
        where('userId', '==', user.uid)
      ),
      async (querySnapshot) => {
        const savedIds = querySnapshot.docs.map(doc => doc.data().snippetId);
  ...
  const filteredAndSortedSnippets = savedSnippets
    .filter(snippet => {
      // Search filter
      if (searchQuery) {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
            Sign in to view your saved snippets
          </h2>
          <p className="text-gray-600 dark:text-gray-400">
            Save snippets to access them later
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex-1 lg:ml-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {loading ? (
            ...
            <EmptyState
              icon={HiBookmark}
              title={searchQuery || filterBy !== 'all' ? 'No saved snippets found' : 'No saved snippets yet'}
              description={
